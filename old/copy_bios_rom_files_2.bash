...
get_system_batocera_roms_directory(){
  if [[ ${SYSTEM_TO_BATOCERA_ROMS_SUBDIR_MAP["$1"]+isset} ]]; then
...
get_system_emudeck_roms_directory(){
  if [[ ${SYSTEM_TO_EMUDECK_ROMS_SUBDIR_MAP["$1"]+isset} ]]; then
...
get_system_esde_roms_directory(){
  if [[ ${SYSTEM_TO_ESDE_ROMS_SUBDIR_MAP["$1"]+isset} ]]; then
...
get_system_minui_bios_directory(){
  if [[ ${SYSTEM_TO_MINUI_BIOS_SUBDIR_MAP["$1"]+isset} ]]; then
...
get_system_minui_roms_directory(){
  if [[ ${SYSTEM_TO_MINUI_ROMS_SUBDIR_MAP["$1"]+isset} ]]; then
...
get_system_muos_roms_directory(){
  if [[ ${SYSTEM_TO_MUOS_ROMS_SUBDIR_MAP["$1"]+isset} ]]; then
...
get_system_onion_roms_directory(){
  if [[ ${SYSTEM_TO_ONION_ROMS_SUBDIR_MAP["$1"]+isset} ]]; then
...
get_system_spruce_roms_directory(){
  if [[ ${SYSTEM_TO_SPRUCE_ROMS_SUBDIR_MAP["$1"]+isset} ]]; then
    echo "$SPRUCE_ROOT_DIR/Roms/${SYSTEM_TO_SPRUCE_ROMS_SUBDIR_MAP["$1"]}"
    return 0
  fi
  return 1
}
